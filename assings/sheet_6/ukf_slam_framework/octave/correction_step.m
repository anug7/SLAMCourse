...
for i = 1:m
  ...
  % sigma_points(1:3) contains robot pose points.
  ...
  for j = 1:(2*n + 1)
    delX = landmarkXs(j) - sigma_points(1, j);
    delY = landmarkYs(j) - sigma_points(2, j);
    rtq = sqrt(delX * delX + delY * delY);
    theta = normalize_angle(atan2(delY, delX) - sigma_points(3, j));
    z_points(:, j) = [rtq; theta]; 
    zm(1) = zm(1) + rtq * wm(j);
    sines = sines + wm(j) * sin(theta);
    cosines = cosines + wm(j) * cos(theta);
  endfor
  
  zm(2) = normalize_angle(atan2(sines, cosines));
  ...
  for j = 1:(2 * n + 1)
    ...
    sigma_x_z = sigma_x_z + (wm(j) * diff_x) * diff_z';
