...
for i = 1:numParticles
  ...
  for j = 1:m
    ...
    if (particles(i).landmarks(l).observed == false)
      ...
      landmark_pos = [ particles(i).pose(1) + z(j).range * cos(normalize_angle(z(j).bearing + particles(i).pose(3))) ; 
                       particles(i).pose(2) + z(j).range * sin(normalize_angle(z(j).bearing + particles(i).pose(3))) ];
      
      particles(i).landmarks(l).mu = landmark_pos;
      % get the Jacobian with respect to the landmark position
      [h, H] = measurement_model(particles(i), z(j));

      % TODO: initialize the EKF for this landmark
      sigma = inv(H) * Qt * (inv(H))';
      particles(i).landmarks(l).sigma = sigma;
      ...
      Kt = particles(i).landmarks(l).sigma * H' * inv(Q);
      
      % TODO: compute the error between the z and expectedZ (remember to normalize the angle)
      diff = [z(j).range; z(j).bearing] - expectedZ;
      diff(2) = normalize_angle(diff(2));
      
      % TODO: update the mean and covariance of the EKF for this landmark
      particles(i).landmarks(l).mu = particles(i).landmarks(l).mu + Kt * diff;
      particles(i).landmarks(l).sigma = (identity - Kt* H) * particles(i).landmarks(l).sigma;
      
      % TODO: compute the likelihood of this observation, multiply with the former weight
      %       to account for observing several features in one time step
      % |2*pi*Q| = compute determinant
      landmark_weight = 1/sqrt(det(2 * pi * Q)) * exp(-0.5 * diff' * inv(Q) * diff);
      particles(i).weight = particles(i).weight * landmark_weight; 
