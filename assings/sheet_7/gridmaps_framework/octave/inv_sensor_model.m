...
log_odd_free = prob_to_log_odds(probFree);
log_odd_end = prob_to_log_odds(probOcc);
for sc=1:columns(laserEndPntsMapFrame)
        ...
        [X, Y] = bresenham([robPoseMapFrame'; laserEndPntsMapFrame(:, sc)']);
        for i=2:size(X, 2) - 1
          mapUpdate(X(i), Y(i)) = log_odd_free;
        endfor
        %mapUpdate(X(2:end-1), Y(2:end-1)) = log_odd_free;
        mapUpdate(laserEndPntsMapFrame(:, sc)(1), laserEndPntsMapFrame(:, sc)(2)) = log_odd_end;
        %TODO: add them to freeCells
        %freeCells_X = [freeCells_X, X(2:end-1)];
        %freeCells_Y = [freeCells_Y, Y(2:end-1)];
        %occCells_X = [occCells_X, laserEndPntsMapFrame(:, sc)(1)];
        %occCells_Y = [occCells_Y, laserEndPntsMapFrame(:, sc)(2)];

endfor

%TODO: update the log odds values in mapUpdate for each laser endpoint according to probOcc.
%mapUpdate(occCells_X, occCells_Y) = log_odd_end;

%TODO: update the log odds values in mapUpdate for each free cell according to probFree.
%mapUpdate(freeCells_X, freeCells_Y) = log_odd_free;
